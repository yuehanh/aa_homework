...
def star_wars
    ...
    # hint: use 'select' and 'where'
    Movie
      .where("title LIKE 'Star Wars%'").select(:id, :title, :yr)
end
...
def below_average_years
    ...
    Movie.where('score < 5').group(:yr).order('COUNT(*) DESC').select(:yr, 'COUNT(*) AS bad_movies')
end
...
def uma_movies
    ...
    Movie.joins(:actors).where(actors: { name: 'Uma Thurman' } ).order(:yr).select(:id, :title, :yr)
end
